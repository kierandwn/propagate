...
inputs control(states x, double t) {
  ...
  vector3 omega_ref = attitude_err.matrix() * omega(sigma_ref, sigma_dot_ref); // body frame
  vector3 omega_dot_ref = attitude_err.matrix() * omega_dot(sigma_ref, sigma_dot_ref, sigma_dot_dot_ref, omega_ref);
